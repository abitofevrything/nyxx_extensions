...
class Pagination extends NyxxPlugin<NyxxGateway> {
  ...
  Future<void> beforeClose(NyxxGateway client) async {
    ...
    if (clientStates != null) {
      await Future.wait(clientStates.map((state) async {
        ...
        if (state.options?.disableOnClientClose ?? options.disableOnClientClose ?? true) {
          state.isDisabled = true;
  ...
  Future<MessageBuilder> generate(
    ...
    return factories(builders, options: options, startIndex: startIndex, userId: userId);
  }
  ...
  Future<MessageBuilder> split(
    ...
    final potentialSplices = splitAt.allMatches(content).map((match) => match.start);
    ...
    return generate(
      ...
      (index) => buildChunk!(content.substring(splices[index], splices[index + 1])),
      options: options,
      userId: userId,
    );
  }
  ...
  Future<MessageBuilder> splitEmbeds(
    ...
    return split(
      ...
      splitAt: splitAt,
      maxLength: maxLength,
      buildChunk: (content) async => MessageBuilder(embeds: [await buildChunk!(content)]),
      options: options,
      userId: userId,
    );
  }
...
class _PaginationState {
  ...
  static String generateId() => 'nyxx_pagination/${DateTime.now().millisecondsSinceEpoch}/${idCounter++}';
  ...
  _PaginationState(this.pagination, this.options, this.builders, this.currentIndex, this.userId);
  ...
  void addControls(MessageBuilder builder, int index) {
    ...
    if (builder.components case final components?
        when components.any(
          (element) => element.components.any(
            (element) => element is ButtonBuilder && {jumpToStartId, jumpToEndId, previousId, nextId}.contains(element.customId),
          ),
        )) {
      return;
    }
    ...
    builder
      ...
      ..components!.add(ActionRowBuilder(components: [
        if (showJumpToEnds)
          ButtonBuilder(
            style: options?.jumpToStartStyle ?? pagination.options.jumpToStartStyle ?? ButtonStyle.secondary,
            label: options?.jumpToStartLabel ?? pagination.options.jumpToStartLabel ?? '<<',
            emoji: options?.jumpToStartEmoji ?? pagination.options.jumpToStartEmoji,
            customId: jumpToStartId,
            isDisabled: isDisabled || index == 0,
          ),
        ButtonBuilder(
          style: options?.previousStyle ?? pagination.options.previousStyle ?? ButtonStyle.primary,
          label: options?.previousLabel ?? pagination.options.previousLabel ?? '<',
        ...
        if (showPageIndex)
          ButtonBuilder(
            style: options?.pageIndexStyle ?? pagination.options.pageIndexStyle ?? ButtonStyle.secondary,
            label: '${index + 1}/${builders.length}',
            customId: 'nyxx_pagination/page_index_display',
            isDisabled: true,
          ),
        ButtonBuilder(
          style: options?.nextStyle ?? pagination.options.nextStyle ?? ButtonStyle.primary,
        ...
        if (showJumpToEnds)
          ButtonBuilder(
            style: options?.jumpToEndStyle ?? pagination.options.jumpToEndStyle ?? ButtonStyle.secondary,
            label: options?.jumpToEndLabel ?? pagination.options.jumpToEndLabel ?? '>>',
